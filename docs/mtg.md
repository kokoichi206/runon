# MTG メモ

記録日: 2026-05-31

## 地図との連携

- Garmin
  - ラウンドトリップルーティング
  - Garmin connect iq
  - garmin 235J にはない
- Google map と連携できる
- 朝走るのがいいらしい

## トレーニングプラン

- 今の garmin
  - 全然ペースが合ってない
  - 一般的なことしか言ってこない
- 理想
  - 直近のレースで VO2max がわかる
  - そこから目標ペースの VO2max の値に伸ばす方法を提案してくれる
  - 心拍の継続的なモニタリングして、改善提案・メニューの組み直しを提案してくれる
- 目標設定も手伝ってくれる

---

関連ドキュメント:

- 周回路の距離計算: [spec-round-trip.md](./spec-round-trip.md)
- トレーニング計画の仕様: [spec-training-planner.md](./spec-training-planner.md)
- トレーニング計画の計算ロジック: [training-plan-logic.md](./training-plan-logic.md)
- Garmin / Strava 連携（データ取得）の調査: [garmin-strava-integration.md](./garmin-strava-integration.md)
