...
const serverEnvSchema = z.object({
  ...
  OVERPASS_ENDPOINT: z.preprocess(
    emptyStringToUndefined,
    z.string().url().optional(),
  ),
  // OSM 利用エチケット用の User-Agent。
  OVERPASS_USER_AGENT: z.preprocess(
    emptyStringToUndefined,
  ...
  STRAVA_REDIRECT_BASE: z.preprocess(
    emptyStringToUndefined,
    z.string().url().optional(),
  ),
});
...
export const isStravaConfigured = (): boolean =>
  serverEnv.STRAVA_CLIENT_ID !== undefined &&
  serverEnv.STRAVA_CLIENT_SECRET !== undefined;
